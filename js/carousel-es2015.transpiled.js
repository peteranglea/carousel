...
var Carousel = function () {
	...
	_createClass(Carousel, [{
		...
		value: function init() {
			...
			var markup = "";
			// Make thumbnail row
			markup += '<div class="carousel-thumbs">' + '<div class="carousel-thumbs-inner">' + '<ul class="clearfix">';
			...
			// Make content area
			markup += '<div class="carousel-content">' + '<div class="carousel-content-inner">' + '<div class="carousel-content-text stage-left"></div>' + '<div class="carousel-content-text current">' + this.data[0].content + '</div>' + '<div class="carousel-content-text stage-right"></div>' + '</div>' + '</div>';

			// Make nav
			markup += '<div class="carousel-nav">' + '<ul class="clearfix">' + '<li class="nav-left">' + '<a href="javascript:;">&larr; Previous</a>' + '</li>' + '<li class="nav-right">' + '<a href="javascript:;">Next &rarr;</a>' + '</li>' + '</ul>' + '</div>';
			...
			this.container.find('.nav-right').click(function () {
				obj.advanceStory('next');
			});
			this.container.find('.nav-left').click(function () {
				obj.advanceStory('prev');
			});
		...
		key: 'goToStory',
		value: function goToStory(key, direction) {
			var obj = this;
		...
		key: 'advanceStory',
		value: function advanceStory(direction) {
			if (direction == 'next') {
				var nextIcon = this.container.find('.carousel-thumbs a.current').parent('li').next().find('a');
				if (nextIcon.length > 0) nextIcon.click();else this.container.find('.carousel-thumbs li:first-child a').click();
			} else if (direction == 'prev') {
				var prevIcon = this.container.find('.carousel-thumbs a.current').parent('li').prev().find('a');
				if (prevIcon.length > 0) prevIcon.click();else this.container.find('.carousel-thumbs li:last-child a').click();
			}
...
//# sourceMappingURL=carousel-es2015.transpiled.js.map
